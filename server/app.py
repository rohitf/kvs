...
if REPLICAS_WANTED is not None:
    ...
    REPLICAS_WANTED = 0
REPLICAS = VIEWS[:REPLICAS_WANTED] # Stores and updates list of replicas, no proxies
...
def get(key):
    ...
    if IS_REPLICA:
        ...
        if need_update:
            success, status_code = kv.put(key, current_max_value, current_max_VC, current_max_timestamp)
            if success:
    ...
    # Proxy
        ...
        for current_replica in REPLICAS:
            try:
                A = requests.get("http://" + current_replica + "/kv-store/" + key, data={'causal_payload': client_CP}, timeout = 2)
                status_code = A.status_code
                response = A.json()
                if status_code == 404:
                    message = {"result" : "error", "msg": "Key does not exist"}
                else:
                    message = {"result": response['result'], "value": response['value'], "node_id": MY_ID, "causal_payload": response['causal_payload'], "timestamp": response['timestamp']}
...
def put(key):
    if not request.json:
        ...
        if IS_REPLICA:
            # If the client's payload isn't equal or more recent to current payload, return failure
            ...
            if not app_funct.compareVC(client_CP, my_vc):
                ...
                if app_funct.equalityVC(client_CP, current_max_VC):
                    ...
                    message = {"result": "success", "value": val, "node_id": MY_ID, "causal_payload": app_funct.deparseVC(client_CP), "timestamp": str(client_timestamp)}
                    return (json.dumps(message), status_code, {'ContentType': 'application/json'})
                    ...
                    message = {"result": "failure", "value": current_max_value, "node_id": MY_ID, "causal_payload": app_funct.deparseVC(client_CP), "timestamp": str(current_timestamp)}
                    return (json.dumps(message), status_code, {'ContentType': 'application/json'})
        else:
            send_data = {'causal_payload': client_CP, 'val' : val}
            for current_replica in REPLICAS:
                try:
                    A = requests.put("http://" + current_replica + "/kv-store/" + key, data= send_data, timeout = 2)
                    status_code = A.status_code
                    response = A.json()
                    message = {"result": response['result'], "value": response['value'], "node_id": MY_ID, "causal_payload": response['causal_payload'], "timestamp": response['timestamp']}
                    return (json.dumps(message), status_code, {'ContentType': 'application/json'})
    ...
    if type == "add":
        ...
        if len(REPLICAS) < REPLICAS_WANTED:
            ...
            for i in REPLICAS:
                ...
                if IS_REPLICA and i is not REPLICAS[MY_ID]:
                    try:
                        requests.get("http://" + i + "/add/", data={'update_ip': update_ip}, timeout=5)
                    except requests.exceptions.Timeout:
                        continue
        # else if proxy
        ...
        if update_ip in REPLICAS:
            ...
            for i in REPLICAS:
                if IS_REPLICA and i is not REPLICAS[MY_ID]:
                    try:
                        requests.put("http://" + i + "/remove/", data={'update_ip': update_ip}, timeout=5)
                    except requests.exceptions.Timeout:
                        continue
...
def add():
    ...
    # Redundant check for replica or proxy because ?? just to avoid sending request to self in update_view
...
def getGossip():
    ...
    for key in sent_d:
        ...
        if my_d_val is None:
            result, status_code = kv.put(key, sent_d_val, sent_vc_val, sent_ts_val)
            continue
...
def gossip(gossip_ipp):
    ...
    d, vc, timestamp = json.dumps(d), json.dumps(vc), json.dumps(timestamp)
    try:
        resp = requests.post("http://" + gossip_ipp + "/kv-store/gossip", data = {"d": d, "vc" : vc, "timestamp" : timestamp })
        print("RESP")
        print(vars(resp))
        resp = resp.json()
...
def ping():
    while True:
        reqs = [grequests.get("http://" + node_address + "/hey", timeout=5) for node_address in REPLICAS]
...
def ping_failed(request, exception):
    ...
    for m in REPLICAS:
        try:
            requests.put("http://" + m + "/kv-store/update_view?type=remove", data={'url': url}, timeout=5)
            print("SUCC")
            break
        except:
            print("CAN't find")
